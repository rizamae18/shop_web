...
$selected_category = isset($_GET['category']) ? $_GET['category'] : 'all_clothing';
...
if (isset($conn)) {
    $db_products = mysqli_query($conn, "SELECT * FROM products WHERE is_active=1 AND category IN ('mens_fashion','womens_fashion') ORDER BY created_at DESC");
    if ($db_products) {
...
foreach ($products as $product) {
    if ($selected_category == 'all_clothing' || $product['category'] == $selected_category) {
    ...
    <main class="container">
        <section class="clothing-controls">
            <div class="sub-categories">
                <a href="Clothing.php" class="pill <?php if($selected_category=='all_clothing') echo 'active'; ?>">All Clothing</a>
                <a href="Clothing.php?category=mens_fashion" class="pill <?php if($selected_category=='mens_fashion') echo 'active'; ?>">Men's Fashion</a>
                <a href="Clothing.php?category=womens_fashion" class="pill <?php if($selected_category=='womens_fashion') echo 'active'; ?>">Women's Fashion</a>
                <a href="Electronics.php" class="pill">Electronics</a>
            </div>
            <p class="item-count">Showing <strong><?php echo count($shown_products); ?></strong> <?php echo strtolower(categoryTitle($selected_category)); ?> items</p>
        </section>

        <section class="product-grid">
            <?php if (count($shown_products) == 0) { ?>
            ...
            <div class="product-card">
                ...
                <div class="product-info">
                    <p class="label"><?php echo strtoupper(categoryTitle($product['category'])); ?></p>
                    ...
                    <div class="card-btns">
                        <form action="BuyNow.php" method="POST" class="buy-now-form">
                            ...
                            <input type="hidden" name="redirect" value="Clothing.php?category=<?php echo urlencode($selected_category); ?>">
                        ...
                        <form action="add_to_cart.php" method="POST" class="cart-form">
                            ...
                            <input type="hidden" name="image" value="<?php echo htmlspecialchars($product['image']); ?>">
                            <input type="hidden" name="category" value="<?php echo strtoupper(categoryTitle($product['category'])); ?>">
                            <input type="hidden" name="redirect" value="Clothing.php?category=<?php echo $selected_category; ?>">
